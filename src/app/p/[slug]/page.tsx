...
function getAvatarColor(name: string): string {
  const colors = ['#006A4E', '#1a7a5e', '#2d6a8f', '#7c3aed', '#b45309', '#0369a1', '#be185d']
  let hash = 0
...
export default async function ProductPage({
  ...
  const { data: product } = await supabase
    .from('products')
    .select('id, nom, description, prix_fcfa, photo_url, slug, actif, sellers(id, nom, whatsapp)')
    .eq('slug', slug)
    .single()
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div
        ...
        style={{ backgroundColor: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(8px)', borderBottom: '1px solid #f3f4f6' }}
      >
        <Link
          href="/"
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-1 mt-0.5">
                  ...
                  >
                    Vendeur vérifié · {formatWhatsapp(seller.whatsapp)}
                  </span>
                </div>
              </div>
            </div>
